# PostgreSQL Mini-Lab (Step-by-Step CRUD)
...
3. Confirm `.env` and `config/postgresql.properties` are filled with lab values.

## Step 1 - Connectivity check
Run:
```bash
python labs/postgresql/01_connect.py
```
...
Run:
```bash
python labs/postgresql/02_create.py
```
...
Run:
```bash
python labs/postgresql/03_read.py
```
...
Run:
```bash
python labs/postgresql/04_update.py
```
...
Run:
```bash
python labs/postgresql/05_delete.py
```
...
Run:
```bash
python labs/postgresql/06_verify.py
```
Expected: verify step confirms the record is absent.

## Common mistakes and fixes
- **Config missing**: fill `.env` and `config/postgresql.properties`; scripts print missing keys.
