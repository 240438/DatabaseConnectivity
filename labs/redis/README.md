# Redis Mini-Lab (Step-by-Step CRUD)
...
3. Confirm `.env` and `config/redis.properties` are filled with lab values.

## Step 1 - Connectivity check
Run:
```bash
python labs/redis/01_connect.py
```
...
Run:
```bash
python labs/redis/02_create.py
```
...
Run:
```bash
python labs/redis/04_update.py
```
...
Run:
```bash
python labs/redis/05_delete.py
```
...
Run:
```bash
python labs/redis/06_verify.py
```
Expected: verify step confirms the record is absent.

## Common mistakes and fixes
- **Config missing**: fill `.env` and `config/redis.properties`; scripts print missing keys.
