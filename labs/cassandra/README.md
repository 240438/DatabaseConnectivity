# Cassandra Mini-Lab (Step-by-Step CRUD)
...
3. Confirm `.env` and `config/cassandra.properties` are filled with lab values.

## Step 1 - Connectivity check
Run:
```bash
python labs/cassandra/01_connect.py
```
...
Run:
```bash
python labs/cassandra/02_create.py
```
...
Run:
```bash
python labs/cassandra/03_read.py
```
...
Run:
```bash
python labs/cassandra/04_update.py
```
...
Run:
```bash
python labs/cassandra/05_delete.py
```
...
Run:
```bash
python labs/cassandra/06_verify.py
```
Expected: verify step confirms the record is absent.

## Common mistakes and fixes
- **Config missing**: fill `.env` and `config/cassandra.properties`; scripts print missing keys.
