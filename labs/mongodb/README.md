# MongoDB Mini-Lab (Step-by-Step CRUD)
...
3. Confirm `.env` and `config/mongodb.properties` are filled with lab values.

## Step 1 - Connectivity check
Run:
```bash
python labs/mongodb/01_connect.py
```
...
Run:
```bash
python labs/mongodb/02_create.py
```
...
Run:
```bash
python labs/mongodb/03_read.py
```
...
Run:
```bash
python labs/mongodb/04_update.py
```
...
Run:
```bash
python labs/mongodb/05_delete.py
```
...
Run:
```bash
python labs/mongodb/06_verify.py
```
Expected: verify step confirms the record is absent.

## Common mistakes and fixes
- **Config missing**: fill `.env` and `config/mongodb.properties`; scripts print missing keys.
